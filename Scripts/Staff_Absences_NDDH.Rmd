...
Daily_Absences_All_Staff <-
  read_excel(
    "Data/Data_from_Claire/Staff_Absences_NDDH_20200403.xlsx",
    "Daily_Absences_All_Staff",
    col_names = TRUE
  ) %>% 
...
Daily_Absences_All_Staff$Historical <- 0.034
...
load("Data/Data_from_Claire/ScenariosND.RData")
...
scenND$RtDoubling <- tail(Daily_Absences_All_Staff$Staff_Absences_Covid_Perc,1) * cumprod(scenND$Doubling_pcnt)

scenND$Rt50 <- tail(Daily_Absences_All_Staff$Staff_Absences_Covid_Perc,1) * cumprod(scenND$Rt50_pcnt)

scenND$Rt75 <- tail(Daily_Absences_All_Staff$Staff_Absences_Covid_Perc,1) * cumprod(scenND$Rt75_pcnt)
```
...
```{r}
cov_long <-
  cov_df %>% pivot_longer(., c("Doubling_ND", "Rt50", "Rt75"))
...
Number_Staff_By_Group <- read_excel("Data/Data_from_Claire/Staff_Absences_NDDH_20200403.xlsx", "Number_Staff_By_Group", col_names = TRUE)
...
```{r}
Absences_By_Group <- read_excel("Data/Data_from_Claire/Staff_Absences_NDDH_20200403.xlsx", "Absences_By_Group",col_names = TRUE) %>% 
...
#ggsave("Outputs/NDDH_Absences_Numbers_TNC_scen_Staff_Groups.png", width = 30, height = 20, units = "cm")

 num_pl_df %>% filter(model == "Rt50" & date == as.Date("2020-03-31") & name == "total_staff_out") %>% summarise(sum = sum(value))
...
#ggsave("Outputs/NDDH_Absences_Numbers_TNC_scen_All_Staff.png", width = 30, height = 20, units = "cm")
...
#ggsave("Outputs/NDDH_Absences_Proportions_TNC_scen_Staff_Groups.png", width = 30, height = 20, units = "cm")
...
#ggsave("Outputs/NDDH_Absences_Proportions_TNC_scen_All_Staff.png", width = 30, height = 20, units = "cm")
```
