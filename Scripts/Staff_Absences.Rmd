...
end_date <- as.Date("2020-04-13")  #End date for projections

in_file <- "Data/Data_from_Claire/Staff_Absences_NDDH_20200403.xlsx" #excel file with data
 
scen_data <- "Data/Data_from_Claire/ScenariosND.RData" #scenario rates
load(scen_data)
scen_in <- ScenariosND
...
scen[1,c("Doubling_pcnt","Rt50_pcnt","Rt75_pcnt")] <- scen[2,c("Doubling_pcnt","Rt50_pcnt","Rt75_pcnt")]


scen$RtDoubling <- tail(Daily_Absences_All_Staff$Staff_Absences_Covid_Perc,1) * cumprod(scen$Doubling_pcnt)

scen$Rt50 <- tail(Daily_Absences_All_Staff$Staff_Absences_Covid_Perc,1) * cumprod(scen$Rt50_pcnt)
...
Number_Staff_By_Group <- read_excel(in_file, "Number_Staff_By_Group", col_names = TRUE) %>% 
...
```{r}
Absences_By_Group <- read_excel(in_file, "Absences_By_Group",col_names = TRUE) %>% 
...
num_plot <-
  ...
  geom_point() +
  geom_smooth(aes(linetype = name), se = F) +
  ...
  theme_bw() +
  ggtitle(paste0("Staff Numbers Absent - ", hosp))


num_plot

ggsave(paste0("Outputs/",hosp, "_",Sys.Date(),"_Absences_Numbers_Staff_Groups.png"), width = 30, height = 20, units = "cm")

 num_pl_df %>% filter(model == "Rt50" & date == as.Date("2020-04-01") & name == "total_staff_out") %>% summarise(sum = sum(value))
...
ggsave(paste0("Outputs/",hosp, "_",Sys.Date(),"_Absences_Numbers_All_Staff.png"), width = 30, height = 20, units = "cm")
...
ggsave(paste0("Outputs/",hosp, "_",Sys.Date(),"_Absences_Proportions_Staff_Groups.png"), width = 30, height = 20, units = "cm")
...
ggsave(paste0("Outputs/",hosp, "_",Sys.Date(),"_Absences_Proportions_All_Staff.png"), width = 30, height = 20, units = "cm")
