...
params:
  hospital: "RDE"
  in_file: "Staff_Absences_RDE_20200408.xlsx"
  end_date: !r as.Date("2020-04-29")
...
#### Modelled Forecasts\
Staff Absences for `r params$hospital` under `r length(scenarios)` modelled forecasts:
...
group_df_lag <- group_df %>%
  ...
  mutate( nrows = n(),
    ...
    cov_staff_return = ifelse(nrows > lag, c(rep(0, lag), diff_staff_cov[1:end_lag]), 0))
