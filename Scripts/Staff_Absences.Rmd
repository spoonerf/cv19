...
#### Modelled Forecasts\
Staff Absences for `r hosp` under four modelled forecasts:
...
```
#### Staff Absence Data\
Percentages of `r hosp` staff absences from `r min(Daily_Absences_All_Staff$Date)` to `r max(Daily_Absences_All_Staff$Date)`
...
```{r Group Weights}
staff_weights <-
  read_excel(paste0(in_dir, in_file), "Absences_By_Group", col_names = TRUE) %>%
...
daily_abs_dates <-
  seq(min(cov_long$date), end_date, by = "day")
...
write.csv(group_df_lag, paste0("Outputs/",hosp,"_",format(Sys.Date(), "%Y%m%d"), "_absences_data.csv"), row.names = FALSE)
